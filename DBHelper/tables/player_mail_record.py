...
class PlayerMailRecord(Basic, Base):
    ...
    send_character_id = CustomColumn(Integer, cn="发送人ID",
                                     comment="邮件发送人的 character_id。如果是游戏管理员,需要在设置表中指定游戏管理员的ID;")  # todo
    received_character_id = CustomColumn(Integer, cn="接收人ID", comment="邮件接收人的 character_id")  # todo
    mail_position_index = CustomColumn(Integer, cn="邮件索引", comment="邮件所占用位置的索引,从1开始;")

    give_stuff_type = CustomColumn(Integer, cn="赠送物品类型", bind_type=StuffType,
                                   comment="赠送物品的id。只有未绑定的物品才可以邮寄给别人。暂定最多赠送一件物品。")
